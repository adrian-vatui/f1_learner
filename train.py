...
if __name__ == "__main__":
    ...
    for i_episode in range(episodes_num):
        ...
        last_episodes_max_reward = np.amax(last_10_reward)
        ...
        while not done:
            ...
            skipped_frames += 1
            if skipped_frames == 1:
                if last_episodes_max_reward > 800:  # if the score is good, remove the noise for 10 episodes
                    # get the action from the Neural network
                    action, network_output = agent.get_action(state, training=False)
                else:
                    # get the action from the Neural network
                    action, network_output = agent.get_action(state, training=True)
            ...
            if skipped_frames == SKIP_FRAMES:
                ...
                agent.add_to_buffer(state_before_skip, network_output, skipped_frames_reward, new_state)

                # and train the networks with the new data added in the memory buffer
                agent.train()
                skipped_frames_reward = 0
                skipped_frames = 0
